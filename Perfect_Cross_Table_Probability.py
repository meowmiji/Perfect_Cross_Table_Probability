...
def scores(matches,nteams): # set of all teams' scores
	...
	for i in range(nteams-1):
		matrix[i]=np.concatenate((np.zeros(i+1,dtype=int),
								  matches[i*nteams-i*(i+1)//2:i*nteams-i*(i+1)//2+nteams-i-1]))
...
def calculate_prob(nteams):
	...
	print('The probability of %d teams is %d / %d = %.4f%%' %(nteams,ordered,total,100*ordered/total))
